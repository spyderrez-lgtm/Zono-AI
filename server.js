...
async function visionChat(message, images, history) {
    ...
    // Keep vision requests deliberately tiny. Qwen 3.6 27B charges 2048
    // input tokens per image, so we send one image, the user's question,
    // and a short instruction instead of the full Zono history/knowledge.
    const image = images[0];
    if (!image) throw new Error("No readable image was supplied.");
    ...
    const content = [
        {
            ...
            text: [
                ...
                "Look carefully at the attached image and answer the user's question directly.",
                "Read visible text, diagrams, tables and signs when relevant.",
                "Do not describe anything that is not actually visible.",
