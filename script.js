document.addEventListener("DOMContentLoaded", () => {
    ...
    const chatNameCancel = document.getElementById("chatNameCancel");
    ...
    function formatZonoText(text) {
        ...
        value = value.replace(/\*\*(.+?)\*\*/g, '<strong class="zono-highlight">$1</strong>');
    ...
    function typeBotMessage(bubble, text, done) {
        const fullText = plainTextForTyping(text);
        bubble.classList.add("typing-text");
        bubble.textContent = "";

        const caret = document.createElement("span");
        caret.className = "zono-caret";
        ...
        const step = () => {
            ...
            if (index < fullText.length) {
                ...
                bubble.innerHTML = formatZonoText(fullText);
    ...
    let chatNameTargetId = null;

    function openChatNameEditor(chatId) {
        const chat = chats.find(item => item.id === chatId);
        if (!chat || !chatNameModal || !chatNameInput) return;
        chatNameTargetId = chatId;
        chatNameInput.value = chat.title || "New Chat";
        chatNameModal.classList.add("show");
    ...
    function closeChatNameEditor() {
        if (!chatNameModal) return;
        chatNameTargetId = null;
        chatNameModal.classList.remove("show");
        chatNameModal.setAttribute("aria-hidden", "true");
    }

    function saveChatName() {
        if (!chatNameTargetId || !chatNameInput) return;
        const chat = chats.find(item => item.id === chatNameTargetId);
        if (!chat) return closeChatNameEditor();
        const name = chatNameInput.value.replace(/\s+/g, " ").trim();
        if (!name) return;
    ...
    function renderHistory() {
        ...
        chats.forEach(chat => {
            ...
            button.title = "Hold to edit chat name";
            ...
            const startHold = () => {
                ...
                holdTimer = window.setTimeout(() => {
                    ...
                    window.setTimeout(() => button.classList.remove("long-pressed"), 260);
                    openChatNameEditor(chat.id);
                }, 650);
            };
    ...
    function addMessageToDOM(role, content, options = {}) {
        ...
        if (normalizedRole === "bot" && options.animate) {
            typeBotMessage(bubble, content || "", options.onDone);
        } else {
    ...
    async function sendMessage() {
        ...
        try {
            ...
            removeTyping();
            renderActiveChat();
            const botBubbles = chatArea.querySelectorAll('.message.bot .bubble');
            const lastBubble = botBubbles[botBubbles.length - 1];
            if (lastBubble) typeBotMessage(lastBubble, answer);
        } catch (error) {
            removeTyping();
            chat.messages.push({
                role: "bot",
    ...
    chatNameCancel?.addEventListener("click", closeChatNameEditor);
    chatNameSave?.addEventListener("click", saveChatName);
    chatNameModal?.addEventListener("click", event => {
        if (event.target.matches("[data-close-chat-name]")) closeChatNameEditor();
    });
    chatNameInput?.addEventListener("keydown", event => {
        if (event.key === "Enter") saveChatName();
        if (event.key === "Escape") closeChatNameEditor();
    });
    ...
    document.addEventListener("keydown", event => {
        if (event.key === "Escape") {
            if (chatNameModal?.classList.contains("show")) closeChatNameEditor();
